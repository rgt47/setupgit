...
description: "I did not really know how much time I was wasting without proper version control until a single overwritten script cost me three days of work."
...
YAML FRONT MATTER
  [x] title           — filled in
  [x] date            — 2026-02-11
  [x] categories      — git, reproducibility (only 2; consider adding
                         shell, workflow, setup to match sibling posts)
  [x] description     — filled in
  [ ] image           — points to media/images/hero.png, an old
                         placeholder; media/images/hero.jpg is present on
                         disk and unused
  [ ] draft: false    — still draft: true

NARRATIVE INPUTS
  [x] Hook sentence   — lost analysis work, no backup
  [x] Pain point      — no version control, ad hoc file naming
  [x] Motivations     — 6 bullets
  [x] Objectives      — 4 numbered
  [x] What is Git?    — definition + distributed-systems framing
  ...
  [x] See Also        — related posts, course materials, key resources

DELIVERABLES
  [x] Prerequisites and installation commands (inline; no separate
      analysis/configs/ deliverable files for this post)
  [x] Verification commands
  [x] Uninstall / rollback steps

VERSION MATRIX
  [ ] OS + version tested   — noted in prose only ("tested on macOS with
                               Zsh and Git 2.43.0"), no formal table
  [ ] Tool versions pinned  — Git version only; gh CLI version unstated
  [ ] Date of last verification — not present

IMAGES (4 total: hero + 3 ambiance, all in media/images/)
  [x] Hero image (80% width) — genuine (bare branching tree), wired
      to hero.jpg 2026-08-25
  [x] Ambiance image 1 (after Objectives) — done (Gemini/Imagen 3,
      2026-08-25): carbon transfer paper and fountain pen
  [x] Ambiance image 2 (after core workflow) — done (Gemini/Imagen 3,
      2026-08-25): dated accordion-fold file
  [x] Ambiance image 3 (before Lessons Learnt) — genuine (UCSD Geisel
      Library), wired to ambiance3.jpg 2026-08-25
  [ ] media/images/README.md updated — currently documents unrelated
      "placeholder coffee" images, not the actual hero/ambiance files
...
version control
workflows](media/images/hero.jpg){.img-fluid width=80% fig-align="center"}
...
described in [post 15](/posts/15-wf-construct-overview-anchor/). Git is
...
1. Install and configure Git on macOS or Linux, including
   identity setup and GitHub authentication through the
...
4. Adopt a plain, imperative-mood commit message
   convention.
...
![A single sheet of carbon transfer paper laid over a blank
notepad on a wooden desk, with a fountain pen resting beside it,
suggesting an exact, traceable duplicate being
made.](media/images/ambiance1.jpg){.img-fluid height=3.5in}
...
- The GitHub CLI (`gh`), used for authentication and for
  the collaboration steps later in this post
...
On Linux, follow the install instructions at
[cli.github.com](https://cli.github.com/). The `gh` tool
handles GitHub authentication and, later in this post, the
collaboration workflow (creating repositories, opening
...
The simplest way to connect Git to a GitHub account is
the `gh` CLI. Run the login command and follow the
prompts:
...
Choose GitHub.com, choose HTTPS as the preferred
protocol, and let `gh` open a browser to complete the
one-time sign-in. Once this is done, `git push` and
`git pull` against GitHub work without any further setup.
This is the right default for most readers, since it
avoids generating and registering a key pair.
...
SSH keys are a fine alternative once the Git workflow
feels familiar. They avoid a browser step on each new
machine but require generating and registering a key pair
first:
...
and sets up the remote connection automatically. Plain
`git clone https://github.com/username/repo.git` works
the same way once `gh auth login` has been run.

I found that creating a repository on GitHub first (with
a README and `.gitignore` for R) and then cloning it
locally was the most reliable approach for new projects:
...
![A hand-bound accordion-fold paper file, partially expanded on a
desk to reveal several distinct dated sections, each tabbed with a
small colored marker.](media/images/ambiance2.jpg){.img-fluid height=3.5in}
...
This sends the invitation directly from the terminal.
The collaborator gets a notification on GitHub and
accepts it there, or from the terminal with
`gh api` if they prefer, though the notification
click is simpler for a one-time accept.
...
`gh` fills in the base branch and repository
...
`gh pr diff` shows the changes without opening a
browser. Squash-merging keeps the `main` branch history
readable, and `--delete-branch` cleans up the feature
branch on the remote once it is merged. This workflow
keeps the main branch clean and provides a record of
every contribution.
...
Some projects prefix each commit message with a category,
following the Angular project's convention:
...
I tried this for a while and found it added overhead
without much payoff for a single-analyst project. My
default now is the plain imperative style from the
section above, with no prefix:
...
A prefix convention is worth adopting for a team project
where commits need to be filtered or grouped by category.
For solo analysis work, plain imperative messages are
...
If `git push` fails with a permission error, check the
`gh` login status first:
...
If using SSH instead, verify that the key is added to the
agent:
...
the correct name and email, `gh auth status` reports a
logged-in account, and the test commit appears in the
...
| Commit changes | `git commit -m "Add model"` |
...
learning](media/images/ambiance3.jpg){.img-fluid height=3.5in}
...
- Authenticating with `gh auth login` over HTTPS
  eliminates password friction for regular use, and skips
  the SSH key setup step entirely.
...
- The plain imperative-mood commit style used throughout
  this post is a default, not a rule. Prefixed conventions
  like Angular's are common on team projects, and
  consistency within a team matters more than which
...
- [Multi-Laptop macOS Bootstrap](/posts/17-wf-multi-laptop-dotfiles-bootstrap/):
  Managing configuration files with Git
- [Unix Command-Line Workspace Setup for Data Science](/posts/16-wf-unix-workspace-config/):
  Terminal setup that complements a Git workflow, and the
  place to look for git aliases and diff tooling
  (`git-delta`, `lazygit`) not covered in this post

**Course materials:**

- *Git and GitHub for Biostatistics* (the lab's five-day
  intro course) covers the same ground as this post in
  more depth, with exercises. The authentication and
  commit-message conventions in this post now follow that
  course.
- *Git Intermediate* (the three-day follow-on course)
  covers selective staging, rewriting history, and
  recovery, none of which is addressed here.
...
[x] Narrative complete (no [bracketed] placeholders remaining)
    [x] grep '\[' index.qmd returns only legitimate code/links

[x] Reproducibility
    [x] Post correctly states no analysis pipeline applies (git-only
        tutorial); minimum requirements listed

[x] Visual design
    [x] 1 hero image (width=80%, fig-align=center) — done 2026-08-25
    [x] 3 ambiance images (.img-fluid only) — all genuine as of
        2026-08-25 (ambiance1/2 newly generated; ambiance3 rewired)
    [x] media/images/README.md attributes every image actually in use

[ ] Content quality
    [ ] Zero emoji
    [ ] Zero em dashes — two instances of "---" used as an em-dash
        substitute in prose ("...irreversible, and the only...it was
        essential"; "...modest initial investment---perhaps an
        afternoon")
    [ ] Single quotes in prose — several double-quoted phrases remain
        (e.g. "I'll just save copies", "fixed stuff")
...
5. Post 19: [Setting Up Neovim as a Data Science IDE](../wf-neovim-data-science-ide/)
